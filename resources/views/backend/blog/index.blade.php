                ...
                <div class="widget-content widget-content-area br-8">
                    <table id="blog-list" class="table dt-table-hover" style="width:100%">
                        <div class="d-flex justify-content-center">
                            <a href="{{ route('blog.create') }}" class="btn btn-success text-center my-3">Create Blog</a>
                        </div>
                        <thead>
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($blog->blogThumbnail)
                                            <span><img width="40%" src="{{ asset('storage/blog_thumbnail') }}/{{ $blog->blogThumbnail }}" alt="{{ $blog->blogTitle }}"></span>
                                        @else
                                            <span><img width="40%" src="{{ Avatar::create($blog->blogTitle)->toBase64() }}" class="profile-img" alt="avatar" /></span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-left align-items-center">
                                            <div class="d-flex flex-column">
                                                <a href="{{ route('blog.edit', $blog->id) }}" class="text-truncate fw-bold">{{ $blog->blogTitle }}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ $blog->relationshipwithServiceCategory->service_category_title }}</td>
                                    <td>
                                        @if ($blog->created_at->diffInDays(\Carbon\Carbon::now()) < 1)
                                            <span class="badge badge-primary">Recently Updated</span>
                                        @endif
                                        @if ($blog->created_at->diffInDays(\Carbon\Carbon::now()) > 1 && $blog->blogStatus == 'on')
                                            <span class="badge badge-success">Published</span>
                                        @endif
                                        @if ($blog->blogStatus == NULL)
