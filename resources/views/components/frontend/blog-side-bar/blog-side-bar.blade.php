    ...
    <div class="cs-sidebar cs-right_sidebar cs-accent_5_bg_2">
        ...
        <ul>
            @foreach ($serviceCategories as $serviceCategory)
                ...
                <a href="{{ route('blog.category', $serviceCategory->id) }}">{{ $serviceCategory->service_category_title }}</a>
                </li>
            @endforeach
        </ul>
        </div>
        ...
        <ul class="cs-recent_posts">
            @foreach ($blogs as $blog)
                ...
                <div class="cs-recent_post">
                    <a href="{{ route('blog.details',$blog->id) }}" class="cs-recent_post_thumb">
                    <div class="cs-recent_post_thumb_in cs-bg" data-src="@if ($blog->blogThumbnail == NULL) {{ asset('storage') }}/blog_thumbnail/nullImage.jpg @else {{ asset('storage') }}/blog_thumbnail/{{ $blog->blogThumbnail }} @endif"></div>
                    </a>
